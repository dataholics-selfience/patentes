  ...
  const loadPatentHistory = async () => {
    ...
    try {
      ...
      const history = querySnapshot.docs.map(doc => {
        ...
        if (Array.isArray(resultado) && resultado.length > 0 && resultado[0].output) {
          try {
            parsedResult = JSON.parse(resultado[0].output);
          } catch (e) {
            console.error('Erro ao parsear output do histórico:', e);
          }
        }
        ...
        return {
          ...
          analises_patentes: parsedResult?.analises_patentes || parsedResult?.patentes || [],
          metricas_chave: parsedResult?.metricas_chave || {},
          relatorio_executivo: parsedResult?.relatorio_executivo || {}
        };
      });
