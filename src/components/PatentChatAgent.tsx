...
const PatentChatAgent = ({ patentHistory, isOpen, onClose }: PatentChatAgentProps) => {
  ...
  const sendMessageToAgent = async (message: string, sid: string) => {
    ...
    try {
      const response = await fetch('https://primary-production-2e3b.up.railway.app/webhook/chat-patentes', {
        method: 'POST',
        headers: {
