...
const PatentChatAgent = ({ patentHistory, isOpen, onClose, environment = 'production' }: PatentChatAgentProps) => {
  ...
  const sendMessageToAgent = async (message: string, sid: string) => {
    ...
    try {
      ...
      console.log('[ChatAgent] Resposta do webhook:', data);

      let assistantMessage = '';
      if (Array.isArray(data) && data.length > 0 && data[0]?.output) {
        assistantMessage = data[0].output;
        console.log('[ChatAgent] Mensagem extraida do array:', assistantMessage);
      } else if (data?.output) {
        assistantMessage = data.output;
        console.log('[ChatAgent] Mensagem extraida do objeto:', assistantMessage);
      } else if (typeof data === 'string') {
        assistantMessage = data;
        console.log('[ChatAgent] Mensagem como string:', assistantMessage);
      } else {
        console.error('[ChatAgent] Formato de resposta nao reconhecido:', data);
        assistantMessage = 'Desculpe, nao consegui processar sua solicitacao.';
      }

      console.log('[ChatAgent] Adicionando mensagem ao chat:', assistantMessage);
