...
const PatentConsultation = ({ checkTokenUsage, tokenUsage }: PatentConsultationProps) => {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState<'pharma' | 'advanced'>('advanced');
  ...
  const [advancedPatentEnvironment, setAdvancedPatentEnvironment] = useState<'production' | 'test'>('production');
  const [advancedPatentSearchData, setAdvancedPatentSearchData] = useState<{nome_comercial: string, nome_molecula: string} | null>(null);
  ...
  const handleAdvancedPatentResult = (data: any, env?: 'production' | 'test', searchData?: {nome_comercial: string, nome_molecula: string}) => {
    setAdvancedPatentData(data);
    setAdvancedPatentEnvironment(env || 'production');
    setAdvancedPatentSearchData(searchData || null);
  };
  ...
  if (advancedPatentData) {
    return (
      <AdvancedPatentReport
        ...
        environment={advancedPatentEnvironment}
        nomeComercial={advancedPatentSearchData?.nome_comercial}
        nomeMolecula={advancedPatentSearchData?.nome_molecula}
      />
    );
  }
